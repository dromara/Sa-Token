...
import cn.dev33.satoken.integration.boot2.support.IntegrationInterceptorSupport;
...
/**
 * 路由鉴权集成测试配置：注册 SaRouter 路由拦截规则（对应 /rt/** 测试端点）。
 */
@Configuration
public class RouterAuthConfig implements WebMvcConfigurer {

    /** 注册上下文桥接拦截器与路由鉴权拦截器 */
    @Override
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(IntegrationInterceptorSupport.contextBridgeInterceptor()).addPathPatterns("/**");
        ...
        registry.addInterceptor(new SaInterceptor(handle -> {})
                ...
                .setAuth(handle -> {
                    ...
                    SaRouter.match("/rt/getInfo16", () -> SaHolder.getResponse().redirect("/rt/getInfo3"));
