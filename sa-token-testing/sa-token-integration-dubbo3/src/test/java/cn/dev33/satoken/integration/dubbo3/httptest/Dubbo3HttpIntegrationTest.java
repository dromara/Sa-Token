 ...
 */
package cn.dev33.satoken.integration.dubbo3.httptest;

import cn.dev33.satoken.integration.dubbo3.IntegrationDubbo3Application;
...
/**
 * Dubbo3 真 RPC：本机 dubbo://，会话下传 / 回传，Same-Token 开着。
 */
@SpringBootTest(classes = IntegrationDubbo3Application.class, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
public class Dubbo3HttpIntegrationTest {

	@Autowired
	TestRestTemplate rest;

	/** 容器启动前预留一个空闲端口，Provider 监听、Consumer 直连都用它，避免固定端口撞车 */
	@DynamicPropertySource
	static void dubboPort(DynamicPropertyRegistry registry) throws IOException {
		int port;
		try (ServerSocket socket = new ServerSocket(0)) {
			port = socket.getLocalPort();
		}
		String portValue = String.valueOf(port);
		registry.add("dubbo.protocol.port", () -> portValue);
		registry.add("test.dubbo.port", () -> portValue);
	}
