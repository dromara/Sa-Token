...
2. 在Spring环境中, 如果同时配置了`WebMvcConfigurer`和`WebMvcConfigurationSupport`时, 也会导致拦截器失效.
   ...
   **常见场景**: swagger相关静态资源开放, 很多教程会在`WebMvcConfigurationSupport`中设置`addResourceHandlers`方法以开放静态资源映射, 同时Sa-Token相关配置添加了`WebMvcConfigurer`配置`addInterceptors`方法注册拦截器, 这样就会导致拦截器失效. 
