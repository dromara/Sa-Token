...
- core：
	...
	- 新增：新增 `SaRequest` / `SaResponse` / `SaStorage` 创建策略。 fix: [#841](https://github.com/dromara/Sa-Token/issues/841)
...
- 插件：
	...
	- 新增：新增 `sa-token-alone-redisson` 独立连接插件及 Spring Boot 3/4 示例。  **[重要]**  fix: [#710](https://github.com/dromara/Sa-Token/issues/710)、 fix: [#ICE2V0](https://gitee.com/dromara/sa-token/issues/ICE2V0)
	- 新增：新增 `sa-token-alone-redis-by-spring-boot4` 插件及示例，实现 Spring Boot 4 下权限缓存与业务缓存分离。  **[重要]**  
	- 新增：`SaTokenDaoForRedisson` 支持指定 Codec（默认 `StringCodec`），update 方法改用 `setAndKeepTTL` 原子保留原过期时间。 **[重要]**  fix: [#791](https://github.com/dromara/Sa-Token/issues/791)
	- 新增：RedisTemplate Dao 支持重写 `wrapKey` 自定义键前缀。 fix: [#956](https://github.com/dromara/Sa-Token/issues/956)
	- 修复：`sa-token-redis-template` 及 jdk-serializer 在 update key 时 TTL 偏移的问题； **[重要]**  fix: [#I80P5O](https://gitee.com/dromara/sa-token/issues/I80P5O) 、[#ICWJOQ](https://gitee.com/dromara/sa-token/issues/ICWJOQ)
	- 修复：`sa-token-redis-template-jdk-serializer` KEEPTTL 泛型编译错误。
	- 修复：redis-template `searchData` 由 KEYS 改为 SCAN。  **[重要]**  fix: [#702](https://github.com/dromara/Sa-Token/issues/702)
...
- starter：
	- 修复：Context Filter 注册 REQUEST+ASYNC，修复 SSE/Flux 流式返回上下文未初始化。 **[重要]**  fix: [#IC4XFE](https://gitee.com/dromara/sa-token/issues/IC4XFE) 、[#ICB9OJ](https://gitee.com/dromara/sa-token/issues/ICB9OJ)
	- 修复：修复 Boot3+ WebFlux/Gateway `setStatus` NoSuchMethodError。 fix: [#IIAW1A](https://gitee.com/dromara/sa-token/issues/IIAW1A) 、[#916](https://github.com/dromara/Sa-Token/issues/916) 、[#IHRIBY](https://gitee.com/dromara/sa-token/issues/IHRIBY)
...
- SSO：
	...
	- 修复：`buildServerAuthUrl` 识别 URL 编码后的 back 参数，back 为空时跳过追加，并增强重定向编码/解码健壮性。 fix: [#842](https://github.com/dromara/Sa-Token/issues/842)
...
- 文档：
	...
	- 新增：文档版本选择器改为动态加载（`all-version-common.js`）。 merge: [pr 928](https://github.com/dromara/Sa-Token/pull/928)  fix: [#900](https://github.com/dromara/Sa-Token/issues/900)
	- 新增：新增源码运行指南文档。
	- 新增：新增 `SECURITY.md` 安全策略及联系邮箱。  **[重要]**  merge: [pr 933](https://github.com/dromara/Sa-Token/pull/933)  fix: [#932](https://github.com/dromara/Sa-Token/issues/932) 、[#877](https://github.com/dromara/Sa-Token/issues/877)
	- 新增：新增「Sa-Token 集成 Demos 示例大全下载」文档。
	- 新增：导航栏增加安全推荐与案例下拉菜单。
	- 优化：文档站点整体优化（侧边栏、首页、捐赠页、导航交互、API 手册等）。
	- 同步：同步公众号文章列表、博客列表、赞助者名单、企业登记案例。
	- 补充：补充 SSE/Flux 流式返回常见问题；补充按配置决定是否启用 Redis 的示例。 fix: [#I9I4ZG](https://gitee.com/dromara/sa-token/issues/I9I4ZG)
