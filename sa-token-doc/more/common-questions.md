...
7. 如果 Maven 镜像源使用了阿里云等国内镜像源，可能因同步不及时导致 SA-Token 的子依赖没有成功载入（不会报错，程序也能正常启动，但是会让 SA-Token 工作不正常），尝试去掉镜像源，还原为国外官方源（https://repo.maven.apache.org/maven2/）
8. 如果以上步骤排除无误后依然报错，请直接提 issue 或者加入QQ群求助。
