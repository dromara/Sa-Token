...
| maxLoginCount			| int		| 12		| 同一账号最大登录数量，-1代表不限 （只有在 `isConcurrent=true`，`isShare=false` 时此配置才有效），[详解](/use/config#配置项详解：maxLoginCount)	|
...
| tokenSessionCheckLogin	| Boolean	| true	| 获取 `Token-Session` 时是否必须登录 （如果配置为true，会在每次获取 `Token-Session` 时校验是否登录），[详解](/use/config#配置项详解：tokenSessionCheckLogin)		|
...
| serverUrl			| String	| null				| 配置 Server 端主机总地址，拼接在 `authUrl`、`signoutUrl`、`pushUrl` 属性前面，用以简化各种 url 配置，参考：[详解](/sso/sso-questions#问：模式三配置一堆-xxx-url-，有办法简化一下吗？)	|
...
| isHttp			| Boolean	| false				| 是否打开模式三（此值为 true 时将使用 http 请求：校验 ticket 值、单点注销、拉取数据getData），参考：[详解](/use/config#配置项详解：isHttp) 	|
| isSlo				| Boolean	| true				| 是否打开单点注销功能							|
