...
description: "Sa-Token 整合 Thymeleaf：引入 sa-token-thymeleaf，注册 SaTokenDialect 后可在模板用 sa:login、sa:hasRole、sa:hasPermission 标签做登录与权限判断。也可注入 stp 全局变量，在页面调用 StpLogic/SaSession API。"
---
