...
description: "OAuth2 Client 域名校验：AllowRedirectUris 是授权回调白名单，生产禁止配 *。否则攻击者可改 /oauth2/authorize 的 redirect_uri 把 code 劫持到自己的站。应写成具体回调地址，名单外 URL 无法下发授权码。通配 * 只允许出现在末尾。"
---
...
- 反例：`http://user@sa-token.com`
- 反例：`http://sa-oauth-client.com@sa-token.com`
