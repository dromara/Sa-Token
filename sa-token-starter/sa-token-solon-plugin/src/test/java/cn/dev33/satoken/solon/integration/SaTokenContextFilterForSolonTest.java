...
/**
 * {@link SaTokenContextFilterForSolon} 上下文写入与清理测试
 */
@SaTokenTest
public class SaTokenContextFilterForSolonTest {
	...
	@Test
	public void doFilter_setAndClearContext() throws Throwable {
		TestSolonContext ctx = SolonTestHelper.newGetContext("/ctx");
		AtomicBoolean seenInChain = new AtomicBoolean(false);
		SaTokenContextFilterForSolon filter = new SaTokenContextFilterForSolon();

		filter.doFilter(ctx, c -> seenInChain.set(SaHolder.getRequest() != null));
