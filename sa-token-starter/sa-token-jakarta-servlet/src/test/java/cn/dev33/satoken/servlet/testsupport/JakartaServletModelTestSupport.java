...
/**
 * Jakarta Servlet 模型单测公共辅助：注册 Jakarta 版 SaStrategy，并在临时上下文中执行断言逻辑。
 */
public final class JakartaServletModelTestSupport {

    private JakartaServletModelTestSupport() {
    }

    /** 注册 Jakarta Servlet 版 Request/Response/Storage 创建策略 */
    ...
    /** 在 Sa-Token Jakarta Servlet 上下文中执行一段逻辑，结束后自动清理上下文 */
    public static void withContext(MockHttpServletRequest request, MockHttpServletResponse response, Runnable action) {
        ...
        SaTokenContextJakartaServletUtil.setContext(request, response, (SaFunction) action::run);
    }

    /** 在 Sa-Token Jakarta Servlet 上下文中执行一段带返回值的逻辑，结束后自动清理上下文 */
    public static <T> T withContext(MockHttpServletRequest request, MockHttpServletResponse response, SaRetGenericFunction<T> action) {
        ensureServletStrategy();
        return SaTokenContextJakartaServletUtil.setContext(request, response, action);
    }
